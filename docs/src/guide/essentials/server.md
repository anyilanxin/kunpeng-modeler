...
import { defineConfig } from '@kunpeng/vite-config';
...
项目中默认自带了基于 `axios` 封装的基础的请求配置，核心由 `@kunpeng/request` 包提供。项目没有过多的封装，只是简单的封装了一些常用的配置，如有其他需求，可以自行增加或者调整配置。针对不同的app，可能是用到了不同的组件库以及`store`,所以在应用目录下的`src/api/request.ts`文件夹下，有对应的请求配置文件,如`web-antd`项目下的`src/api/request.ts`文件,可以根据自己的需求进行配置。
 ...
 * 该文件可自行根据业务逻辑进行调整
 */
import type { HttpResponse } from '@kunpeng/request';

import { useAppConfig } from '@kunpeng/hooks';
import { preferences } from '@kunpeng/preferences';
import {
  authenticateResponseInterceptor,
  errorMessageResponseInterceptor,
  RequestClient,
} from '@kunpeng/request';
import { useAccessStore } from '@kunpeng/stores';
...
```ts
import { defineOverridesPreferences } from '@kunpeng/preferences';
