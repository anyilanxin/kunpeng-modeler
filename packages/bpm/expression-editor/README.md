# @kunpeng/expression-editor

Embeddable Editor
for [FEEL expressions](https://docs.camunda.io/docs/components/modeler/feel/what-is-feel/).

## Usage

To get started, create a feel-editor instance:

```JavaScript
import ExpressionEditor from '@kunpeng/expression-editor';

const editor = new ExpressionEditor({
  container
});
```

Configure the FEEL dialect (expression or unary tests):

```JavaScript
const editor = new ExpressionEditor({
  container,
  dialect: 'unaryTests' // defaults to 'expression'
});
```

You can provide a starting document and listen for changes:

```JavaScript
const editor = new ExpressionEditor({
  container,
  onChange,
  onKeyDown,
  onLint,
  value
});
```

### Variables

You can provide a variables array that will be used for auto completion. Nested structures are
supported. The Variables need to be in the following format:

```JavaScript
const editor = new ExpressionEditor({
  container,
  variables: [
    {
      name: 'variablename to match',
      detail: 'optional inline info',
      info: 'optional pop-out info',
      entries: [
        {
          name: 'nested variable',
          ...
        }
      ]
    }
  ]
});
```

The variables can be updated on the instance via `ExpressionEditor#setVariables()`:

```javascript
editor.setVariables([
  {
    name: 'newName',
    detail: 'new variable inline info',
    info: 'new pop-out info',
  },
]);
```

### Content attributes

You can provide attributes which will be set on the content editable div:

```javascript
const editor = new ExpressionEditor({
  container,
  contentAttributes: {
    'aria-label': 'Expression editor',
  },
});
```

### Events

You can listen to events fired by the editor:

```javascript
const editor = new ExpressionEditor({
  container,
});

editor.on('lint', ({ diagnostics }) => console.log(diagnostics));
```

## Hacking the Project

To get the development setup make sure to have [NodeJS](https://nodejs.org/en/download/) installed.
As soon as you are set up, clone the project and execute

```
npm install
npm start
```

## License

MIT
