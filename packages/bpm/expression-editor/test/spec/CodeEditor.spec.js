import ExpressionEditor from '../../src/index.js';
...
describe('CodeEditor', function () {
  ...
  (singleStart && singleStart !== 'camunda' ? it.only : it)(
    'should render',
    async function () {
      // when
...
return
  ...
  (singleStart === 'camunda' ? it.only : it)(
    'should render with camunda dialect',
    async function () {
      // when
      const initialValue =
        '"At Camunda, you can" + escape.`variables with backticks`';
  ...
  it('should use full container height', async function () {
    ...
    // then
    expect(editor._cmEditor.contentDOM.clientHeight).to.be.eq(
  ...
  describe('#focus', function () {
    ...
    it('should scroll into view', function () {
      ...
      const editorContainer =
        scrollContainer.querySelector('#editor-container');
  ...
  describe('autocompletion', function () {
    it('should complete variables', function () {
      ...
      const variables = [{ name: 'foobar' }, { name: 'baz' }];
    ...
    it('should complete variables (after set)', async function () {
      ...
      // when
      editor.setVariables([{ name: 'foobar' }, { name: 'baz' }]);
      startCompletion(cm);
    ...
    it('should complete variables (after update)', async function () {
      ...
      const editor = new ExpressionEditor({
        container,
        value: initialValue,
        variables: [{ name: 'foobar' }, { name: 'baz' }],
      });
      ...
      // when
      editor.setVariables([{ name: 'foobaz' }]);
      startCompletion(cm);
    ...
    describe('tooltip position', function () {
      ...
      const variables = [
        {
          ...
          info: () => {
            const html = domify(
              '<div data-test="oversized-description" style="width: 100px; height: 100px"><div>',
            );
            return html;
          },
        },
      ...
      beforeEach(function () {
        tooltipContainer =
          domify(`<div data-test="tooltip-container" style="width: 500px; height: 500px; position: relative;">
                                    <div data-test="feel-editor" style="width: 50px; height: 20px; position: absolute; bottom: 0; right: 0;"></div>
                                  </div>`);
        feelContainer = tooltipContainer.querySelector(
          '[data-test="feel-editor"]',
        );
      ...
      it('should position tooltips inside container', function () {
        ...
        return expectEventually(() => {
          const tooltip = container.querySelector(
            '[data-test="oversized-description"]',
          );
...
/**
 * @param {ExpressionEditor} editor
 */
function getCm(editor) {
  return /** @type {import('@codemirror/view').EditorView} */ (
    editor._cmEditor || editor
  );
}
...
async function lint(editor) {
  ...
  return Promise.race([timeout(1000), lintPromise]);
}
