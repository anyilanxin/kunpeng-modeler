...
```javascript
import { kunpengBuiltins } from '@camunda/feel-builtins';
...
```javascript
import { cmFeelLinter } from '@kunpeng/expression-lint';
...
const myEditor = new EditorView({
  state: EditorState.create({
    doc: '',
    extensions: [linter(cmFeelLinter())],
  }),
});
```
...
To get the development setup make sure to have [NodeJS](https://nodejs.org/en/download/) installed. As soon as you are set up, clone the project and execute
