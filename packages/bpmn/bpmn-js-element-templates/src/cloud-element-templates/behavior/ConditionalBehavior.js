...
export default class ConditionalBehavior extends CommandInterceptor {
  ...
  constructor(eventBus, elementTemplates, commandStack, bpmnFactory, injector) {
    ...
    this.postExecute(
      [
        'element.updateProperties',
        'element.updateModdleProperties',
        'propertiesPanel.kunpeng.changeTemplate',
        'element.move',
      ],
  ...
  _applyConditions(context, event) {
    const { element, newTemplate, oldTemplateWithConditions } = context;
  ...
  _ensureConditional(context) {
    const { element, newTemplate } = context;
...
function hasDifferentPropertyBindings(sourceTemplate, targetTemplate) {
  return (
    hasNewProperties(sourceTemplate, targetTemplate) ||
    hasRemovedProperties(sourceTemplate, targetTemplate)
  );
}
...
function hasNewProperties(sourceTemplate, targetTemplate) {
  ...
  return properties.some(
    (targetProp) =>
      !sourceTemplate.properties.find((sourceProp) =>
        compareProps(sourceProp, targetProp),
      ),
  );
}
...
function hasRemovedProperties(oldTemplate, newTemplate) {
  ...
  return oldProperties.some(
    (property) => !findPropertyWithBinding(newTemplate, property),
  );
}
...
function findPropertyWithBinding(template, prop1) {
  return template.properties.some((prop2) =>
    areBindingsEqual(prop1.binding, prop2.binding),
  );
}
...
function normalizeBinding(binding) {
  if (binding.type === KUNPENG_TASK_DEFINITION_TYPE_TYPE) {
    return {
      ...binding,
      type: KUNPENG_TASK_DEFINITION,
      property: 'type',
    };
  }
...
function equals(a, b) {
  return (
    JSON.stringify(a, normalizeReplacer) ===
    JSON.stringify(b, normalizeReplacer)
  );
}
...
function isMetaUpdate(event, context) {
  return (
    event === 'element.updateProperties' &&
    Object.keys(context.properties).every((key) =>
      ['kunpeng:modelerTemplate', 'kunpeng:modelerTemplateVersion'].includes(
        key,
      ),
    )
  );
