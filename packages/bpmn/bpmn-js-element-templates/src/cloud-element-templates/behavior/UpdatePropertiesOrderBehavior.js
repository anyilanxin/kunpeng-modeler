...
export default class UpdateTemplatePropertiesOrder extends CommandInterceptor {
  ...
  _updatePropertiesOrder(context) {
    ...
    if (kunpengProperties) {
      this._updateKunpengPropertiesOrder(kunpengProperties, templateProperties, commands, context);
    }
    ...
    if (additionDynamics) {
      this._updateadditionDynamicsOrder(additionDynamics, templateProperties, commands, context);
    }
  ...
  _updateKunpengPropertiesOrder(kunpengProperties, templateProperties, commands, context) {
    const findIndex = (properties, propertyToFind) =>
      properties.findIndex(prop =>
        prop.binding.type == 'kunpeng:property' && prop.binding.name === propertyToFind.get('name')
      );

    const properties = kunpengProperties.get('properties');
  ...
  _updateInputOrder(ioMapping, templateProperties, commands, context) {
    ...
    const findIndex = (properties, propertyToFind) =>
      properties.findIndex(prop =>
        prop.binding.type == 'kunpeng:input' && prop.binding.name === propertyToFind.get('target')
      );
  ...
  _updateOutputOrder(ioMapping, templateProperties, commands, context) {
    ...
    const findIndex = (properties, propertyToFind) =>
      properties.findIndex(prop =>
        prop.binding.type == 'kunpeng:output' && prop.binding.source === propertyToFind.get('source')
      );
  ...
  _updateadditionDynamicsOrder(additionDynamics, templateProperties, commands, context) {

    const findIndex = (properties, propertyToFind) =>
      properties.findIndex(prop =>
        prop.binding.type == 'kunpeng:taskHeader' && prop.binding.key === propertyToFind.get('key')
      );

    const headers = additionDynamics.get('kunpeng:values');
