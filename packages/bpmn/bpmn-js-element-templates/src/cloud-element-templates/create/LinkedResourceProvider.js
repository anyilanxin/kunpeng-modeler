...
export default class LinkedResourcePropertyBindingProvider {
  static create(element, options) {
    ...
    const linkedResources = ensureExtension(element, 'kunpeng:LinkedResources', bpmnFactory);

    let linkedResource = linkedResources.get('values').find(linkedResource => linkedResource.get('linkName') === linkName);

    if (!linkedResource) {
      linkedResource = createElement('kunpeng:LinkedResource', { linkName }, bo, bpmnFactory);
