import { find, isString, isUndefined, reduce } from 'min-dash';
...
export default class ElementTemplates {
  ...
  get(elementOrTemplateId, version) {
    ...
    if (isUndefined(elementOrTemplateId)) {
      ...
      if (
        ...
        templates[elementOrTemplateId][version]
      ) {
      ...
      return this.get(
        this._getTemplateId(element),
        this._getTemplateVersion(element),
      );
    }
  }
  ...
  getDefault(element) {
    return (
      find(this.getAll(element), function (template) {
  ...
  _coerceEngines(engines) {
    ...
    return reduce(
      engines,
      (validEngines, version, engine) => {
        const coercedVersion = coerce(version);

        if (!isSemverValid(coercedVersion)) {
          console.error(
            new Error(
              `Engine <${engine}> specifies unparseable version <${version}>`,
            ),
          );
