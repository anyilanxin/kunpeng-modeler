...
import { satisfies as isSemverCompatible } from 'semver';
...
export function findTemplates(
  ...
  const { latest: includeLatestOnly, deprecated: includeDeprecated } = options;

  const getVersions = (template) => {
    const { latest, ...versions } = template;
    return includeLatestOnly
      ? !includeDeprecated && latest && latest.deprecated
        ? []
        : latest
          ? [latest]
          : []
  ...
  if (isObject(elementOrTemplateId)) {
    ...
    return (
      filter(findTemplates(null, templatesIndex, options), function (template) {
  ...
  if (isString(elementOrTemplateId)) {
    return (
      templatesIndex[elementOrTemplateId] &&
      getVersions(templatesIndex[elementOrTemplateId])
    );
  }
...
export function findOutputParameter(inputOutput, binding) {
  ...
  return parameters.find(function (parameter) {
    ...
    if (
      !definition ||
      binding.scriptFormat !== definition.get('camunda:scriptFormat')
    ) {
      return false;
    }
...
export function findCamundaErrorEventDefinition(element, errorRef) {
  const errorEventDefinitions = findExtensions(element, [
