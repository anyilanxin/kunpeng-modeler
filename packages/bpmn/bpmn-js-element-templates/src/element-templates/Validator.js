import { filter, forEach, isArray, isString } from 'min-dash';

import semverCompare from 'semver-compare';

import { validRange as isSemverRangeValid } from 'semver';

import {
  validateKunpeng as validateAgainstSchema,
  getKunpengSchemaVersion as getTemplateSchemaVersion,
} from '@kunpeng/element-templates-validator';
...
export class Validator {
  ...
  _validateTemplate(template) {
    ...
    if (
      schemaVersion &&
      semverCompare(SUPPORTED_SCHEMA_VERSION, schemaVersion) < 0
    ) {
    ...
    if (this._templatesById[id] && this._templatesById[id][version]) {
      if (version === '_') {
        ...
        return this._logError(
          `template id <${id}> and version <${version}> already used`,
          template,
        );
      }
    }
    ...
    const { errors: schemaErrors, valid } = schemaValidationResult;
  ...
  _validateElementType(template) {
    if (template.elementType && template.appliesTo) {
      ...
      if (!appliesTo.find((type) => this._isType(elementType, type))) {
        return this._logError(
          `template does not apply to requested element type <${elementType}>`,
          template,
        );
      }
      ...
      for (const sourceType of appliesTo) {
        if (!this._canMorph(sourceType, elementType)) {
          return this._logError(
            `can not morph <${sourceType}> into <${elementType}>`,
            template,
          );
        }
      }
  ...
  _isType(type, baseType) {
    ...
    return (
      moddleType &&
      baseType in this._moddle.getElementDescriptor(moddleType).allTypesByName
    );
  }
  ...
  _canMorph(sourceType, targetType) {
    ...
    const baseType = MORPHABLE_TYPES.find((type) =>
      this._isType(sourceType, type),
    );
...
export function filteredSchemaErrors(schemaErrors) {
  return filter(schemaErrors, (err) => {
    const { instancePath, keyword } = err;
    ...
    if (
      keyword === 'type' &&
      instancePath &&
      !instancePath.startsWith('/scopes/')
    ) {
      return true;
    }
