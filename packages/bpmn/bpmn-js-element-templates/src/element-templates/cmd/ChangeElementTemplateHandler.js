...
import { find, isString, isUndefined, keys, without } from 'min-dash';
...
export default class ChangeElementTemplateHandler {
  ...
  preExecute(context) {
    ...
    if (newTemplate) {
      ...
      this._updateCamundaInputOutputParameterProperties(
        element,
        oldTemplate,
        newTemplate,
      );
      ...
      this._updateCamundaErrorEventDefinitionProperties(
        element,
        oldTemplate,
        newTemplate,
      );
  ...
  _updateCamundaErrorEventDefinitionProperties(
    ...
    const oldErrorEventDefinitions = findExtensions(element, [
    ...
    newProperties.forEach((newProperty) => {
      const oldProperty = findOldProperty(oldTemplate, newProperty),
        oldEventDefinition =
          oldProperty && findOldBusinessObject(extensionElements, oldProperty),
      ...
      // (3) create new event definition + error
        ...
        const rootElement = getRoot(getBusinessObject(element)),
          ...
          newEventDefinition = createCamundaErrorEventDefinition(
            newProperty.value,
            newError,
            extensionElements,
            bpmnFactory,
      ...
      commandStack.execute('element.updateModdleProperties', {
        ...
        properties: {
          values: without(extensionElements.get('values'), (value) =>
            oldErrorEventDefinitions.includes(value),
          ),
        },
      });
    }
  ...
  _updateCamundaExecutionListenerProperties(element, newTemplate) {
    ...
    const oldExecutionListeners = findExtensions(element, [
    ...
    const newExecutionListeners = newProperties.map((newProperty) => {
      ...
      return createCamundaExecutionListener(
        newBinding,
        propertyValue,
        bpmnFactory,
      );
  ...
  _updateCamundaFieldProperties(
    ...
    newProperties.forEach((newProperty) => {
      const oldProperty = findOldProperty(oldTemplate, newProperty),
        oldField =
          oldProperty && findOldBusinessObject(businessObject, oldProperty),
        newBinding = newProperty.binding;
      ...
      // (3) add new fields
        ...
        const newCamundaFieldInjection = createCamundaFieldInjection(
          newBinding,
          newProperty.value,
          bpmnFactory,
        );
        ...
        commandStack.execute('element.updateModdleProperties', {
          ...
          properties: {
            [propertyName]: [
              ...businessObject.get(propertyName),
              newCamundaFieldInjection,
            ],
          },
        });
      }
    ...
    if (oldFields.length) {
      commandStack.execute('element.updateModdleProperties', {
        ...
        moddleElement: businessObject,
        properties: {
          [propertyName]: without(businessObject.get(propertyName), (value) =>
  ...
  _updateCamundaInOutProperties(element, oldTemplate, newTemplate) {
    ...
    const extensionElements = this._getOrCreateExtensionElements(
      getSignalEventDefinition(element) || element,
    );

    const oldInsAndOuts = findExtensions(extensionElements, [
      'camunda:In',
      'camunda:Out',
    ]);
    ...
    newProperties.forEach((newProperty) => {
      ...
      if (oldProperty && oldInOurOut) {
        ...
        if (
          (oldBinding.local && !newBinding.local) ||
          (!oldBinding.local && newBinding.local)
        ) {
      ...
      // (3) add new ins and outs
        ...
        if (newBindingType === 'camunda:in') {
          newInOrOut = createCamundaIn(
            newBinding,
            newPropertyValue,
            bpmnFactory,
          );
        } else if (newBindingType === 'camunda:out') {
          newInOrOut = createCamundaOut(
            newBinding,
            newPropertyValue,
            bpmnFactory,
          );
        } else if (newBindingType === 'camunda:in:businessKey') {
          newInOrOut = createCamundaInWithBusinessKey(
            newPropertyValue,
            bpmnFactory,
          );
        }
   ...
   * @param {Object} newTemplate
   */
  _updateCamundaInputOutputParameterProperties(
    element,
    oldTemplate,
    newTemplate,
    businessObject,
    ...
    const newProperties = newTemplate.properties.filter((newProperty) => {
      ...
      return (
        newBindingType === 'camunda:inputParameter' ||
        newBindingType === 'camunda:outputParameter'
      );
    });
      ...
      commandStack.execute('element.updateModdleProperties', {
        ...
        properties: {
          inputParameters: without(
            inputOutput.get('inputParameters'),
      ...
      commandStack.execute('element.updateModdleProperties', {
        ...
        properties: {
          outputParameters: without(
            inputOutput.get('outputParameters'),
   ...
   * @param {ModdleElement} businessObject
   */
  _updateCamundaPropertyProperties(
    element,
    oldTemplate,
    newTemplate,
    businessObject,
    ...
    newProperties.forEach((newProperty) => {
      ...
      // (3) add new properties
        ...
        const newCamundaProperty = createCamundaProperty(
          newBinding,
          newPropertyValue,
          bpmnFactory,
        );
    ...
    if (oldCamundaProperties.length) {
      commandStack.execute('element.updateModdleProperties', {
        ...
        moddleElement: camundaProperties,
        properties: {
          values: without(camundaProperties.get('values'), (value) =>
            oldCamundaProperties.includes(value),
          ),
        },
      });
    }
    ...
    newProperties.forEach((newProperty) => {
      ...
      if (newBindingName === 'conditionExpression') {
        ...
        if (CAMUNDA_SERVICE_TASK_LIKE.indexOf(newBindingName) !== -1) {
          CAMUNDA_SERVICE_TASK_LIKE.forEach(
            (camundaServiceTaskLikeProperty) => {
  ...
  _updateScopeProperties(element, oldTemplate, newScopeTemplate, newTemplate) {
    ...
    const oldScopeTemplate = findOldScopeTemplate(
      newScopeTemplate,
      oldTemplate,
    );
    ...
    // update camunda:In and camunda:Out properties
    this._updateCamundaInOutProperties(
      element,
      oldScopeTemplate,
      newScopeTemplate,
    );
    ...
    // update camunda:Field properties
    this._updateCamundaFieldProperties(
      element,
      oldScopeTemplate,
      newScopeTemplate,
      scopeElement,
    ...
    // update camunda:Property properties
    this._updateCamundaPropertyProperties(
      element,
      oldScopeTemplate,
      newScopeTemplate,
      scopeElement,
...
function findOldBusinessObject(element, oldProperty) {
  ...
  if (oldBindingType === 'camunda:field') {
    ...
    if (
      !businessObject ||
      !businessObject.get(propertyName) ||
      !businessObject.get(propertyName).length
    ) {
      return;
    }
  ...
  if (oldBindingType === 'camunda:out') {
    return find(businessObject.get('values'), function (oldBusinessObject) {
      return (
        oldBusinessObject.get('source') === oldBinding.source ||
        oldBusinessObject.get('sourceExpression') ||
        oldBinding.sourceExpression
      );
    });
  }

  if (
    oldBindingType === 'camunda:inputParameter' ||
    oldBindingType === 'camunda:outputParameter'
  ) {
    if (is(businessObject, 'camunda:Connector')) {
    ...
    if (oldBindingType === 'camunda:inputParameter') {
      return find(
        businessObject.get('camunda:inputParameters'),
      ...
      return find(
        businessObject.get('camunda:outputParameters'),
        function (oldBusinessObject) {
          if (oldBinding.scriptFormat) {
            ...
            return (
              definition &&
              definition.get('camunda:value') === oldBinding.source
            );
          } else {
  ...
  if (oldBindingType === 'camunda:property') {
    if (
      !businessObject ||
      !businessObject.get('values') ||
      !businessObject.get('values').length
    ) {
      return;
    }
...
function findOldProperty(oldTemplate, newProperty) {
  ...
  if (newBindingType === 'camunda:field') {
    return find(oldProperties, function (oldProperty) {
      ...
      return (
        oldBindingType === 'camunda:field' && oldBindingName === newBindingName
      );
    });
  }
  ...
  if (newBindingType === 'camunda:property') {
    return find(oldProperties, function (oldProperty) {
      ...
      return (
        oldBindingType === 'camunda:property' &&
        oldBindingName === newBindingName
      );
    });
  }
  ...
  if (newBindingType === 'camunda:errorEventDefinition') {
    return find(oldProperties, function (oldProperty) {
      ...
      return (
        oldBindingType === 'camunda:errorEventDefinition' &&
...
function findOldScopeElement(element, scopeTemplate, template) {
  ...
  if (scopeName === 'bpmn:Error') {
    ...
    const errorEventDefinitionBinding = findErrorEventDefinitionBinding(
      template,
      id,
    );
    ...
    const errorEventDefinition = findOldBusinessObject(
      element,
      errorEventDefinitionBinding,
    );
