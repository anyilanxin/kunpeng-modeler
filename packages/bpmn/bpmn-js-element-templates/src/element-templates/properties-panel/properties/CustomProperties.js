...
export function CustomProperties(props) {
  const { element, elementTemplate, injector } = props;

  const translate = injector.get('translate');

  const groups = [];

  const { id, properties, groups: propertyGroups, scopes } = elementTemplate;
...
function addCustomGroup(groups, props) {
  const { element, id, label, properties, scope, templateId } = props;
...
function BooleanProperty(props) {
  const { element, id, property, scope } = props;

  const { description, editable, label } = property;
...
function StringProperty(props) {
  ...
  const { description, editable, label, placeholder } = property;
...
function propertyGetter(element, property, scope) {
  return function getValue() {
    ...
    const { binding, value: defaultValue = '' } = property;
    ...
    if (type === CAMUNDA_ERROR_EVENT_DEFINITION_TYPE) {
      ...
      const errorEventDefinition = findCamundaErrorEventDefinition(
        businessObject,
        errorRef,
      );
    ...
    if (type === CAMUNDA_FIELD_TYPE) {
      ...
      if (camundaField) {
        return (
          camundaField.get('camunda:string') ||
          camundaField.get('camunda:expression')
        );
      } else {
        return '';
      }
...
function propertySetter(bpmnFactory, commandStack, element, property, scope) {
  return function setValue(value) {
    ...
    if (type === CAMUNDA_ERROR_EVENT_DEFINITION_TYPE) {
      ...
      const oldCamundaErrorEventDefinition = findCamundaErrorEventDefinition(
        businessObject,
        errorRef,
      );
      ...
      if (oldCamundaErrorEventDefinition) {
        ...
        const newError = createError(
          ...
          newCamundaErrorEventDefinition = createCamundaErrorEventDefinition(
            value,
            newError,
            extensionElements,
            bpmnFactory,
          ...
          context: {
            ...
            properties: {
              values: [
                ...
                newCamundaErrorEventDefinition,
              ],
            },
          },
        });
    ...
    // camunda:Field
    if (type === CAMUNDA_FIELD_TYPE) {
      const oldCamundaFields = findExtensions(businessObject, [
        'camunda:Field',
      ]);
      ...
      if (oldCamundaFields.length) {
        oldCamundaFields.forEach((camundaField) => {
          if (camundaField.name === name) {
            newCamundaFields.push(
              createCamundaFieldInjection(binding, value, bpmnFactory),
            );
          } else {
      ...
      const values = extensionElements
        .get('values')
        .filter((value) => !oldCamundaFields.includes(value));
    ...
    if (type === CAMUNDA_PROPERTY_TYPE) {
      ...
      if (scope) {
        ...
        camundaProperties = findExtension(
          extensionElements,
          'camunda:Properties',
        );
      }
      ...
      if (!camundaProperties) {
        camundaProperties = createElement(
          'camunda:Properties',
          null,
          businessObject,
          bpmnFactory,
      ...
      const oldCamundaProperty = findCamundaProperty(
        camundaProperties,
        binding,
      );
      ...
      const values = camundaProperties
        .get('values')
        .filter((value) => value !== oldCamundaProperty);
    ...
    if (IO_BINDING_TYPES.includes(type)) {
      ...
      // camunda:InputParameter
      if (type === CAMUNDA_INPUT_PARAMETER_TYPE) {
        const oldCamundaInputParameter = findInputParameter(
          inputOutput,
          binding,
        );

        const newCamundaInputParameter = createInputParameter(
          binding,
          value,
          bpmnFactory,
        );
        ...
        const values = inputOutput
          ...
          .filter((value) => value !== oldCamundaInputParameter);
      ...
      // camunda:OutputParameter
      if (type === CAMUNDA_OUTPUT_PARAMETER_TYPE) {
        const oldCamundaOutputParameter = findOutputParameter(
          inputOutput,
          binding,
        );

        const newCamundaOutputParameter = createOutputParameter(
          binding,
          value,
          bpmnFactory,
        );
        ...
        const values = inputOutput
          ...
          .filter((value) => value !== oldCamundaOutputParameter);
    ...
    if (IN_OUT_BINDING_TYPES.includes(type)) {
      ...
      const values = extensionElements
        .get('values')
        .filter((value) => value !== oldCamundaInOut);
  ...
  return function validate(value) {
    ...
    if (pattern) {
      ...
      if (!matchesPattern(value, pattern)) {
        return (
          message || translate('Must match pattern {pattern}.', { pattern })
        );
      }
    }
  };
