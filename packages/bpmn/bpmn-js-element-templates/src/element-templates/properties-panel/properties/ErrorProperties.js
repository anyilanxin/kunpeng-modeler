...
import { findCamundaErrorEventDefinition, findExtensions } from '../../Helper';
...
export function ErrorProperties(props) {
  ...
  const { binding, label } = property;

  const { errorRef } = binding;

  const businessObject = getBusinessObject(element),
    errorEventDefinitions = findExtensions(businessObject, [
...
function Expression(props) {
  const { errorEventDefinition, id } = props;
