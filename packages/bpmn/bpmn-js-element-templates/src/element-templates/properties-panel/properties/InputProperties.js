...
import { ToggleSwitchEntry } from '@kunpeng/properties-panel';
...
export function InputProperties(props) {
  const { element, index, property, groups } = props;
  ...
  const inputParameter =
    inputOutput && findInputParameter(inputOutput, binding);
  ...
  if (inputParameter) {
    ...
    const item = inputGroup.items.find(
      ({ entries }) => entries[0].parameter === inputParameter,
    );
...
function LocalVariableAssignment(props) {
  const { element, id, property, inputParameter } = props;
