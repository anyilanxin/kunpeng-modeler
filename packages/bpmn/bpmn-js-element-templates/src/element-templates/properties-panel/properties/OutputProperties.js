...
import { TextFieldEntry, ToggleSwitchEntry } from '@kunpeng/properties-panel';

import { containsSpace } from '../../util/validationUtil';

import { findExtension, findOutputParameter } from '../../Helper';
...
export function OutputProperties(props) {
  const { element, index, injector, property } = props;

  const { binding, description, label } = property;
  ...
  const outputParameter =
    inputOutput && findOutputParameter(inputOutput, binding);
...
function ProcessVariableAssignment(props) {
  const { element, id, property, outputParameter } = props;
...
function addOutputParameter(element, property, bpmnFactory, modeling) {
  ...
  if (!extensionElements) {
    ...
    const inputOutput = createInputOutput(
      binding,
      value,
      bpmnFactory,
      extensionElements,
    ...
    update = {
      outputParameters: inputOutput
        .get('camunda:outputParameters')
        .concat(outputParameter),
    };
  }
