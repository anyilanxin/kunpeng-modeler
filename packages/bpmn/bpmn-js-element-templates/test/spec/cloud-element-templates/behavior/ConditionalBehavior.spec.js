...
import { findExtension, findMessage, findSignal, findKunpengSubscription } from 'src/cloud-element-templates/Helper';
...
describe('provider/cloud-element-templates - ConditionalBehavior', function() {
  ...
  describe('update kunpeng:taskDefinition:type and kunpeng:taskDefinition', function() {
    ...
    it('undo', inject(function(commandStack, modeling) {
      ...
      // then
      expect(findExtension(businessObject, 'kunpeng:TaskDefinition')).to.be.undefined;

      // when
      commandStack.undo();

      expect(findExtension(businessObject, 'kunpeng:TaskDefinition')).to.exist;
    }));
  ...
  describe('update kunpeng:ioMapping', function() {
    ...
    it('undo', inject(function(commandStack, modeling) {
      ...
      // assume
      expect(findExtension(businessObject, 'kunpeng:IoMapping')).to.be.undefined;

      // when
      commandStack.undo();

      expect(findExtension(businessObject, 'kunpeng:IoMapping')).to.exist;
  ...
  describe('update kunpeng:taskHeader', function() {
    ...
    it('should add conditional entries', inject(
      async function(modeling) {
        ...
        // then
        expect(additionDynamics).to.have.lengthOf(1);
      })
    );
    ...
    it('should clear additionDynamics if no task headers', inject(
      async function(modeling) {
    ...
    it('undo', inject(function(commandStack, modeling) {
      ...
      // assume
      expect(findExtension(businessObject, 'kunpeng:additionDynamics')).to.exist;

      // when
      commandStack.undo();

      // then
      expect(findExtension(businessObject, 'kunpeng:additionDynamics')).not.to.exist;
    ...
    describe('adding conditional entries', function() {
      ...
      it('should add conditional entries', inject(
        async function(modeling) {
          ...
          expect(zeebeProperties.get('kunpeng:properties')).to.have.lengthOf(1);
        })
      );
      ...
      it('should switch between conditional properties', inject(
        async function(modeling) {
          ...
          // then
          expectKunpengPropertyValue(businessObject, '');
          ...
          // then
          expectKunpengPropertyValue(businessObject, 'nameProp=foobar');
        })
      );
      ...
      it('undo', inject(function(commandStack, modeling) {
        ...
        // assume
        expect(findExtension(businessObject, 'kunpeng:Properties')).to.exist;

        // when
        commandStack.undo();

        // then
        expect(findExtension(businessObject, 'kunpeng:Properties')).not.to.exist;
      }));
  ...
  describe('update bpmn:Message#property', function() {
    ...
    it('should remove conditional entries', inject(
      async function(elementRegistry, modeling) {
        ...
        const property = findExtension(element, 'kunpeng:Properties').get('properties')[0];
    ...
    it('should set kunpeng:modelerTemplate on created message', inject(function() {
      ...
      expect(message.get('kunpeng:modelerTemplate')).to.eql(template.id);
    }));
  });
  ...
  describe('update bpmn:Message#kunpeng:subscription#property', function() {
    ...
    it('undo', inject(function(commandStack, elementRegistry, modeling) {
      ...
      const element = elementRegistry.get('SubscriptionEvent_1');
      const businessObject = getBusinessObject(element);
      const property = findExtension(businessObject, 'kunpeng:Properties').get('properties')[0];
      ...
      // assume
      let subscription = findKunpengSubscription(findMessage(businessObject));
      expect(subscription).to.not.exist;

      // when
      commandStack.undo();

      subscription = findKunpengSubscription(findMessage(businessObject));
      expect(subscription).to.have.property('correlationKey', 'one');
    }));
  ...
  describe('update bpmn:Signal#property', function() {
    ...
    it('should set kunpeng:modelerTemplate on created signal', inject(function() {
      ...
      expect(signal.get('kunpeng:modelerTemplate')).to.eql(template.id);
    }));
  ...
  describe('conditions', function() {
    ...
    describe('feel', function() {
      ...
      it('should keep number property value when condition property is feel-static', inject(function() {
        ...
        // then
        expectKunpengPropertyValueByKey(businessObject, 'text-static');
      }));
      ...
      it('should keep number property value when condition property is feel-optional', inject(function() {
        ...
        // then
        expectKunpengPropertyValueByKey(businessObject, 'text-optional');
      }));
      ...
      it('should keep number property value when condition property is feel-required', inject(function() {
        ...
        // then
        expectKunpengPropertyValueByKey(businessObject, 'text-required');
      }));
    });
    ...
    it('should match when property type is Number but value is a numeric string and condition expects a Number', inject(function() {
      ...
      expectKunpengPropertyValueByKey(businessObject, 'compare-with-number');
    }));
    ...
    it('should match number property with number value when condition has numeric value', inject(function() {
      ...
      // then
      expectKunpengPropertyValueByKey(businessObject, 'compare-number-with-number');
    }));
    ...
    it('should not match when property type is Number but condition expect string', inject(function() {
      ...
      const businessObject = getBusinessObject(element);
      const zeebeProperties = findExtension(businessObject, 'kunpeng:Properties');
      const properties = zeebeProperties.get('kunpeng:properties');
...
function expectTaskHeaderValue(businessObject, value) {
  ...
  expect(additionDynamics).to.have.lengthOf(1);
  expect(additionDynamics[0].value).to.eql(value);
}

function expectKunpengPropertyValue(businessObject, value) {
  const zeebeProperties = findExtension(businessObject, 'kunpeng:Properties');
  const properties = zeebeProperties.get('kunpeng:properties');
...
function expectKunpengPropertyValueByKey(businessObject ,key, value) {
  const zeebeProperties = findExtension(businessObject, 'kunpeng:Properties');
  const properties = zeebeProperties.get('kunpeng:properties');
