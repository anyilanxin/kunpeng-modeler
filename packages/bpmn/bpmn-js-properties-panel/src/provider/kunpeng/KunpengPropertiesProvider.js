import { Group, ListGroup } from '@kunpeng/properties-panel';
...
import {
  ...
  TaskListenersProps,
  TaskScheduleProps,
  ThrowMessageImplementationProps,
  TimerProps,
  VersionTagProps,
} from './properties';
...
import {
  isKunpengServiceTask,
  isMessageEndEvent,
  isMessageThrowEvent,
} from './utils/KunpengServiceTaskUtil';
...
export default class KunpengPropertiesProvider {
  ...
  _getGroups(element) {
    const groups = KUNPENG_GROUPS.map((createGroup) =>
...
function AdHocSubProcessImplementationGroup(element, injector) {
  ...
  const group = {
    ...
    entries: [...AdHocSubProcessImplementationProps({ element })],
    component: Group,
  };
...
function MessagePublishGroup(element, injector) {
  const translate = injector.get('translate');
  const group = {
    id: 'messagePublish',
    label: translate('MessagePublish'),
    entries: [...MessagePublishProps({ element })],
    component: Group,
  };
...
function AdditionGroup(element, injector) {
  const translate = injector.get('translate');
  const group = {
    id: 'additions',
    label: translate('Additions'),
    component: ListGroup,
    ...AdditionProps({ element, injector }),
  };
...
function OutputPropagationGroup(element, injector) {
  ...
  const group = {
    ...
    entries: [...OutputPropagationProps({ element })],
    component: Group,
  };
...
function InputPropagationGroup(element, injector) {
  ...
  const group = {
    ...
    entries: [...InputPropagationProps({ element })],
    component: Group,
  };
...
function BusinessRuleImplementationGroup(element, injector) {
  ...
  const group = {
    ...
    label: translate('Implementation'),
    entries: [...BusinessRuleImplementationProps({ element })],
    component: Group,
  };
...
function ScriptImplementationGroup(element, injector) {
  ...
  const group = {
    ...
    label: translate('Implementation'),
    entries: [...ScriptImplementationProps({ element })],
    component: Group,
  };
...
function ThrowMessageImplementationGroup(element, injector) {
  const translate = injector.get('translate');
  const group = {
    id: 'throwMessageImplementation',
    label: translate('Throw Message Implementation'),
    entries: [...ThrowMessageImplementationProps({ element })],
    component: Group,
  };
...
function OutputCollectionGroup(element, injector) {
  ...
  const group = {
    ...
    entries: [...OutputCollectionProps({ element })],
    component: Group,
  };
...
function ActiveElementsGroup(element, injector) {
  ...
  const group = {
    ...
    entries: [...ActiveElementsProps({ element })],
    component: Group,
  };
...
function ExecutionListenersGroup(element, injector) {
  ...
  const group = {
    ...
    id: 'Kunpeng__ExecutionListeners',
    component: ListGroup,
...
function ExtensionPropertiesGroup(element, injector) {
  ...
  const group = {
    ...
    id: 'Kunpeng__ExtensionProperties',
    component: ListGroup,
    ...ExtensionPropertiesProps({ element, injector, namespace: 'kunpeng' }),
  };
...
function EventConditionGroup(element, injector) {
  ...
  const group = {
    id: 'event-condition',
    label: translate('Condition'),
    entries: [...EventConditionProps({ element })],
    component: Group,
  };
...
function updateTimerGroup(groups, element, injector) {
  ...
  timerEventGroup.entries = [...TimerProps({ element, injector })];
}
...
function updateMultiInstanceGroup(groups, element) {
  ...
  multiInstanceGroup.entries = [...MultiInstanceProps({ element })];
}
...
function removeCompletionGroup(groups, element) {
  if (isKunpengServiceTask(element)) {
...
function replaceEntries(oldEntries, newEntries) {
  const filteredEntries = oldEntries.filter(
    (oldEntry) => !newEntries.find((newEntry) => newEntry.id === oldEntry.id),
  );
