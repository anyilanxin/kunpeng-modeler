...
function ScriptImplementation(props) {
  ...
  const setValue = (value) => {
    ...
    if (value === SCRIPT_IMPLEMENTATION_OPTION) {
      ...
      extensionElementType = 'kunpeng:Script';
...
function getScript(element) {
  ...
  return getExtensionElementsList(businessObject, 'kunpeng:Script')[0];
}

function getAdditionDynamics(element) {
  const businessObject = getBusinessObject(element);

  return getExtensionElementsList(businessObject, 'kunpeng:additionDynamics');
}
...
function resetElement(element, commandStack) {
  ...
  if (taskDefinition) {
    const removed = [ taskDefinition, additionDynamics ].filter(Boolean);
...
function updateExtensionElements(element, extensionElementToAdd, bpmnFactory, commandStack) {
  ...
  if (is(extensionElementToAdd, 'kunpeng:TaskDefinition')) {
    extensionElementValues = without(extensionElements.get('values'), getScript(element));
  } else if (is(extensionElementToAdd, 'kunpeng:Script')) {
    const matcher = extension => isAny(extension, [ 'kunpeng:TaskDefinition', 'kunpeng:additionDynamics' ]);
