...
import { isKunpengUserTask } from '../utils/FormUtil';
...
function addFactory({ bpmnFactory, commandStack, element }) {
  return function(event) {
    ...
    // (2) ensure kunpeng:TaskListeners
    let taskListeners = getTaskListenersContainer(businessObject);

    if (!taskListeners) {
      const parent = extensionElements;

      taskListeners = createElement('kunpeng:TaskListeners', {
        listeners: []
      }, parent, bpmnFactory);
    ...
    // (3) create kunpeng:TaskListener
    const TaskListener = createElement(
      'kunpeng:TaskListener', getDefaultListenerProps(), taskListeners, bpmnFactory
    );
...
function getTaskListenersContainer(element) {
  const TaskListeners = getExtensionElementsList(element, 'kunpeng:TaskListeners');
...
function canHaveTaskListeners(bo, moddle) {
  const TaskListenersDescriptor = moddle.getTypeDescriptor('kunpeng:TaskListeners');
