...
function resolveHeaderEntry(element, node, index, trail, field) {
  ...
  const listener = trail.find(
    (entry) => entry.node && entry.node.$type === 'zeebe:ExecutionListener',
  );

  if (listener) {
    const listenerPrefix = getListEntryId(
      element,
      listener.node,
      listener.index,
    );
...
function resolveListenerEntry(element, node, index, field) {
  if (
    node.$type !== 'zeebe:ExecutionListener' &&
    node.$type !== 'zeebe:TaskListener'
  ) {
    return null;
  }
...
function resolveTimerEntry(node, field) {
  if (
    node.$type !== 'bpmn:TimerEventDefinition' ||
    !TIMER_PROPERTIES.includes(field)
  ) {
    return null;
  }
...
function resolveReferenceNameEntry(businessObject, field) {
  if (
    field === 'errorCode' &&
    hasEventDefinition(businessObject, 'bpmn:ErrorEventDefinition')
  ) {
    return 'errorCode';
  }

  if (
    field === 'escalationCode' &&
    hasEventDefinition(businessObject, 'bpmn:EscalationEventDefinition')
  ) {
...
function hasEventDefinition(businessObject, type) {
  const eventDefinitions =
    businessObject.get && businessObject.get('eventDefinitions');

  return (
    isArray(eventDefinitions) &&
    eventDefinitions.some((definition) => is(definition, type))
  );
}
