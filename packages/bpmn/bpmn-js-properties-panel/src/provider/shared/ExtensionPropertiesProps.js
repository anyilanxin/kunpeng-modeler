...
export function ExtensionPropertiesProps({ element, injector, namespace = 'kunpeng' }) {
...
function getPropertyName(namespace = 'kunpeng') {
  if (namespace === 'kunpeng') {
    return 'properties';
  }
...
export function getProperties(element, namespace = 'kunpeng') {
  const businessObject = getRelevantBusinessObject(element);
  return getExtensionElementsList(businessObject, `${namespace}:Properties`)[ 0 ];
}

export function getPropertiesList(element, namespace = 'kunpeng') {
  const businessObject = getRelevantBusinessObject(element);
  const properties = getProperties(businessObject, namespace);
