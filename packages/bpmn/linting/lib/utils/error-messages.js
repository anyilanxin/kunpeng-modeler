...
function getPropertyValueDuplicatedErrorMessage(report) {
  ...
  let typeString = getTypeString(parentNode || node);

  if (is(node, 'zeebe:TaskHeaders') && every(properties, property => is(property, 'zeebe:Header'))) {
    if (is(node.$parent, 'zeebe:ExecutionListener')) {
      typeString = getTypeString(node.$parent);
    }
...
function getExtensionElementRequiredErrorMessage(report, executionPlatform, executionPlatformVersion) {
  ...
  if (requiredExtensionElement === 'zeebe:UserTask') {
    if (greaterOrEqual(executionPlatformVersion, '8.10')) {
      return `${ getIndefiniteArticle(typeString) } <${ typeString }> with <Implementation: Job worker> managed by Camunda is not supported. Migrate to <Implementation: Camunda user task>.`;
    }
...
function getPropertyNotAllowedErrorMessage(report, executionPlatform, executionPlatformVersion, modeler = 'desktop') {
  ...
  if (is(node, 'zeebe:CalledElement') && property === 'businessId') {
    return getSupportedMessage(`${ getIndefiniteArticle(typeString) } <${ typeString }> with <Business ID>`, executionPlatform, executionPlatformVersion, allowedVersion);
  }
...
function getPropertyValueNotAllowedErrorMessage(report, executionPlatform, executionPlatformVersion, modeler = 'desktop') {
  ...
  if (is(node, 'zeebe:CalledElement') && property === 'businessId') {
    return `${ getIndefiniteArticle(typeString) } <${ typeString }> with a <Business ID> longer than 255 characters is not supported`;
  }
  ...
  if (is(node, 'zeebe:CalledDecision') && property === 'versionTag' && node.get('versionTag')?.startsWith('=')) {
    return getSupportedMessage(`${ getIndefiniteArticle(typeString) } <${ typeString }> with an expression as <Version tag>`, executionPlatform, executionPlatformVersion, allowedVersion);
  }
