...
import { is, isFeel } from '@kunpeng/dmn-js-shared/lib/util/ModelUtil';

import EditableComponent from '@kunpeng/dmn-js-shared/lib/components/EditableComponent';
...
class _LiteralExpressionEditorComponent extends Component {
  ...
  getEditor() {
    return this.isFeel() ? ExpressionEditor : TextEditor;
  }
...
class TextEditor extends EditableComponent {
  render() {
    return <div className={this.getClassName()}>{this.getEditor()}</div>;
  }
}

const LiteralExpressionEditorComponent = withChangeSupport(
  _LiteralExpressionEditorComponent,
