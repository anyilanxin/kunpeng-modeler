...
import ContentEditable from '@kunpeng/dmn-js-shared/lib/components/ContentEditable';
import LiteralExpression from '@kunpeng/dmn-js-shared/lib/components/LiteralExpression';
...
export default class DecisionRulesCellEditorComponent extends Component {
  ...
  render() {
    const { cell, rowIndex, row, col, colIndex } = this.props;
...
class TableCellEditor extends Component {
  ...
  isDefaultExpressionLanguage(businessObject) {
    ...
    return (
      !expressionLanguage || expressionLanguage === defaultExpressionLanguage
    );
  }
  ...
  getExpressionLanguageLabel(businessObject) {
    ...
    return (
      this._expressionLanguages.getLabel(expressionLanguage) ||
  ...
  getEditor() {
    return this.isFEEL() ? ExpressionEditor : ContentEditable;
  }
  ...
  render() {
    const { businessObject, placeholder, value, onChange } = this.props;

    const description = this.getDescription(businessObject);

    const isDefaultExpressionLanguage =
      this.isDefaultExpressionLanguage(businessObject);

    const expressionLanguageLabel =
      this.getExpressionLanguageLabel(businessObject);
    ...
    return (
      <div className="cell-editor">
        {isString(description) && <div className="description-indicator"></div>}
        <Editor
