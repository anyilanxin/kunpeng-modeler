...
export function getComparisonString(comparisonOperator, comparisonValue) {
  return comparisonOperator === 'equals'
    ? `duration("${comparisonValue}")`
    : `${operators[comparisonOperator]} duration("${comparisonValue}")`;
}
...
export function getRangeString(
  ...
  return (
    `${rangeStartChar}duration("${rangeStartValue}")` +
...
function parseRange(text) {
  const { start, end, firstValue, secondValue } = match(
    /^(?<start>[[(])duration\("(?<firstValue>[^"]*)"\)\.\.duration\("(?<secondValue>[^"]*)"\)(?<end>[)\]])$/,
    text,
  );
...
function match(regex, input) {
  const { groups } = regex.exec(input) || { groups: {} };
