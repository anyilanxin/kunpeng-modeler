...
import List from '@kunpeng/dmn-js-shared/lib/components/List';
import ValidatedInput from '@kunpeng/dmn-js-shared/lib/components/ValidatedInput';
import { isInput } from '@kunpeng/dmn-js-shared/lib/util/ModelUtil';
...
const INPUT_ENTRY_VALUES_LABEL = 'Custom values';
const INPUT_VALUES_LABEL = 'Predefined values';
const OUTPUT_VALUES_LABEL = 'Predefined values';
...
export default class SimpleStringEditContextMenuComponent extends Component {
  ...
  render() {
    ...
    const { inputValue, isOutputValueInputChecked, items, unaryTestsType } =
      this.state;
    ...
    return (
      <div class="simple-string-edit context-menu-container">
        <h3 class="dms-heading">{this._translate('Edit string')}</h3>
        ...
        {isInputClause ? (
          <h4 className="dms-heading">{this._translate('Add values')}</h4>
        ) : (
          <h4 className="dms-heading">{this._translate('Set value')}</h4>
        )}
