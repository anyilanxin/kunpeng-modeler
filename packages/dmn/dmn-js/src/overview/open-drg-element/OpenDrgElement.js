...
export default class OpenDrgElement {
  constructor(canvas, elementRegistry, eventBus) {
    ...
    eventBus.on('import.done', () => {
      ...
      if (currentOpenDrgElementId) {
        const currentOpenDrgElement = elementRegistry.get(
          currentOpenDrgElementId,
        );

        if (currentOpenDrgElement) {
          canvas.addMarker(
            currentOpenDrgElement,
            CURRENT_OPEN_DRG_ELEMENT_MARKER,
          );
        }
      }
    });
  ...
  centerViewbox = (element) => {
    ...
    // skip if the canvas hasn't been laid out yet (zero or non-finite size),
    // otherwise SVGMatrix.scale throws "The provided float value is non-finite"
    if (
      !viewbox.outer ||
      !viewbox.outer.width ||
      !viewbox.outer.height ||
      // eslint-disable-next-line unicorn/prefer-number-properties
      !isFinite(viewbox.outer.width) ||
      // eslint-disable-next-line unicorn/prefer-number-properties
      !isFinite(viewbox.outer.height)
    ) {
      return;
    }
