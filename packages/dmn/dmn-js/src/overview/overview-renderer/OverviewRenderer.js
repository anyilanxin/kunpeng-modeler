...
import { getName, is } from '@kunpeng/dmn-js-shared/lib/util/ModelUtil';
...
const DECISION_STROKE_COLOR = '#000';
...
export default function DrdRenderer(eventBus, pathMap, styles, textRenderer) {
  ...
  function initMarkers(svg) {
    function createMarker(id, options) {
      ...
      if (attrs.strokeDasharray === 'none') {
        attrs.strokeDasharray = [10_000, 1];
      }
  ...
  function renderLabel(p, label, options) {
    const text = textRenderer.createText(
      label || '',
      assign({ style: { fill: DEFAULT_STROKE_COLOR } }, options || {}),
    );
  ...
  function renderEmbeddedLabel(p, element, options) {
    ...
    return renderLabel(
      p,
      name,
      assign({ box: element, padding: 5 }, options || {}),
    );
  }
  ...
  const handlers = {
    'dmn:Decision': function (p, element, attrs) {
      ...
      const rect = drawRect(p, element.width, element.height, 0, attrs);

      renderEmbeddedLabel(p, element, {
        align: 'center-middle',
    ...
    'dmn:InputData': function (p, element, attrs) {
      const rect = drawRect(p, element.width, element.height, 22, attrs);
  ...
  function drawShape(parent, element) {
    ...
    return h
      ? h(parent, element)
      : Reflect.apply(BaseRenderer.prototype.drawShape, this, [
          parent,
          element,
        ]);
  }
  ...
  function drawConnection(parent, element) {
    ...
    return h
      ? h(parent, element)
      : Reflect.apply(BaseRenderer.prototype.drawConnection, this, [
          parent,
          element,
        ]);
  }
...
DrdRenderer.$inject = ['eventBus', 'pathMap', 'styles', 'textRenderer'];
